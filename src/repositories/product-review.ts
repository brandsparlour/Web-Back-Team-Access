...
export const retrieveProductReviewsByProductId = async (
  productId: number,
): Promise<Result<IProductReviewDetails[]>> => {
  const connection: PoolConnection = await getDbConnection();
  try {
    const result: IProductReviewDetails[] = await query(
      connection,
      "SELECT * from ProductReviews where product_id = ?",
      [productId],
    );
    ...
    logger.error(
      `at: repositories/product-review/retrieveProductReviewsByProductId => ${err} \n ${JSON.stringify(err)}`,
    );
